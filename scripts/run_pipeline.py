...
given ablation (feature-group/region subset, see ablations.py) on a
given dataset (see config.py DATASETS).
...
Each ablation gets its own directory under RESULTS_DIR:
    ...
    <RESULTS_DIR>/A7/
        ...
        Human_HCT116_eval_results.csv
        Mouse_4T1_feature_importance.png
        Mouse_4T1_model_comparison.png
        Mouse_4T1_top_features.csv
        Mouse_4T1_r2_results.csv
        Mouse_4T1_eval_results.csv

So `results/A7/` has everything for that ablation, across both datasets,
and nothing from any other ablation collides with it. IMPORTANT: this is
what makes ablations safe to run in parallel via SLURM array — if this
routing is ever removed (e.g. reverting to writing straight into
RESULTS_DIR), parallel array tasks will silently overwrite each other's
output files. Do not remove the out_dir = RESULTS_DIR/ablation_id line.

Pipeline order
--------------
    config.py + ablations.py → dataset paths, hyperparameters, ablation specs
        v
    data_loader.py      → load Excel, engineer/cache features, slice by
                          group+region, impute, return arrays
        v
    models.py            → cross-validate (predefined splits) + fit on full data
        v
    model_results.py    → export R² and full-metric results to <out_dir>
...
def run_dataset(path, te_col, label, species, ablation_id, ablation_spec):
    """Run the full pipeline for a single (dataset, ablation) combination."""
    out_dir = str(Path(RESULTS_DIR) / ablation_id)

    print(f"\n{'#'*60}")
    print(f"  Running: {label}  |  Ablation: {ablation_id}  |  spec: {ablation_spec}")
    print(f"  Output dir: {out_dir}")
    print(f"{'#'*60}")

    # 1. load cached features, sliced to this ablation's spec
    X, y, folds, feature_names = load_and_prepare(path, te_col, species, ablation_spec)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Which ablation to run (e.g. A7, B1, C2, D4). Default: run all ablations."
    )
    parser.add_argument(
