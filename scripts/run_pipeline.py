...
-------
    <label>_r2_results.csv            R² only (per-fold + OOF) — legacy format,
                                       also used by the ablation pipeline
    <label>_eval_results.csv          R², MAE, RMSE, Pearson r, Spearman rho
                                       (per-fold + OOF) — full pipeline only
...
def run_dataset(path, te_col, label, out_prefix, species):
    ...
    fitted_models, cv_scores  = fit_models(X, y, folds, label)
     ...
     # 3. export evaluation results to the results folder
    # save_r2_results() is kept for the ablation pipeline; the full
    # pipeline additionally exports MAE / RMSE / Pearson r / Spearman rho
