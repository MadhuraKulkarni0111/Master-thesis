...
def get_importances(fitted_models, feature_names):
    ...
    - Lasso / ElasticNet : absolute value of the learned coefficient.
      The sign tells direction (positive = higher feature → higher TE).
    - RandomForest       : mean decrease in impurity (MDI) across all trees.
    - LightGBM           : total gain — how much each feature reduced the
                           squared error summed across all splits that used it.
                           This is more meaningful than split-count because
                           a feature used once for a very informative split
                           outweighs a feature used many times for trivial ones.
    """
    imp = {}
    ...
    imp["LightGBM"] = pd.Series(
        fitted_models["LightGBM"].feature_importances_,   # uses gain (set above)
        index=feature_names
    )
...
DATASETS = [
    (
        "/mnt/user-data/uploads/Human_data.xlsx",
        "TE_HCT116", "Human_HCT116",
        "/mnt/user-data/outputs/human_hct116"
    ),
    (
        "/mnt/user-data/uploads/Mouse_data.xlsx",
        "TE_4T1", "Mouse_4T1",
        "/mnt/user-data/outputs/mouse_4t1"
    ),
]
